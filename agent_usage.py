...
def _extract_reasoning(response) -> str | None:
    ...
    `content`. Поле сохраняется в истории и отправляется в следующий запрос
    под тем же именем; в консоли наблюдения также видно, что модель «думала»,
    — в том числе на reasoning-only итерациях.
    """
    if response is None:
        return None
